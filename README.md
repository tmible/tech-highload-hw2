...
### Расчёт нагрузки
Если считать, что пользователь разлогинивается после каждого звонка и заходит в приложение только ради них, получается средняя нагрузка на хранилище сессий — `2 411 895 пользователей × 4 раза в сутки / 24 часа / 3600 секунд = 111 RPS`, пиковая — `2 411 895 пользователей × 4 раза в сутки / (2 411 895 / 168 170) = 672 680 RPS`.

Пусть пользователи авторизуются в половине случаев открытия приложения. Тогда данные о пользователе нужны при каждой проверке пароля и при каждом входе в звонок. Итого в среднем `2 411 895 пользователей × 4 раза в сутки × 1,5 (1 — вероятность запроса на подключение к звонку, 0,5 — вероятность запроса данных для авторизации) / 24 часа / 3600 секунд = 167 RPS`, максимально — `2 411 895 пользователей × 4 раза в сутки × 1,5 / (2 411 895 / 168 170) = 1 009 020 RPS`.

Для одного звонка в среднем требуется 8 операций: создание звонка, 4 присоединения к нему, 2 отсоединения, удаление звонка. Отсюда на хранилище звонков средняя нагрузка — `8 × 2 411 895 / 24 / 3600 = 223 RPS`, максимальная — `8 × 2 411 895 / (2 411 895 / 168 170) = 1 345 360 RPS`.
## Прочие технологии
...
### RPS
Для получения и передачи видео и аудио сервис принимает в среднем `2 411 895 пользователей × 4 звонка × 8 запросов (2 на получение + 6 в среднем на передачу) / 24 часа / 3600 секунд = 893 RPS`. Максимально — `2 411 895 пользователей × 4 звонка × 8 запросов / (2 411 895 / 168 170) = 5 381 440 RPS`.  
Авторизации, подлючения к звонкам, отлючения от звонков, создание и удаление звонков создают ещё `((2 411 895 пользователей × 2 (авторизация только в половине случаев)) + (2 411 895 звонков × 4 подключения) + (2 411 895 звонков × 2 отключения) + 2 411 895 созданий + 2 411 895 удалений) / 24 / 3600 = 279 RPS` в среднем, `2 411 895 × 10 / (2 411 895 / 168 170) = 1 681 700 RPS` максимально.
### Трафик
#### Видео
...
Самым распространённым разрешением веб-камеры являяется VGA 640×480 с 4-битным цветом [\[10\]][10]. За среденее разрешение видео с фронтальной камеры смартфона возьмём 854×430. Будем считать количество запросов с десктопов и смартфонов равными. Частоту кадров и для тех, и для тех примем 24 fps. Тогда получаем `(113,08 Кбайт/с × 3600 секунд × 2 411 895 × 2 × 0,6 × 4) + (905,34 Кбайт/с × 3600 секунд × 2 411 895 × 2 × 0,6 × 4) = 4,3 Пбайт + 34,3 Пбайт = 38,6 Пбайт` трафика в сутки. `38,6 Пбайт / 24 / 3600 = 416,5 Гбайт/с` в среднем, `38,6 Пбайт/с / (2 411 895 / 168 170) = 2,7 Пбайт/с` максимально.
...
Для смартфонов:  
`905,34 Кбайт/с × 3600 секунд × 2 411 895 × 2 × 0,6 = 8,6 Пбайт` на получение  
`905,34 Кбайт/с × 3600 секунд × 2 411 895 × 2 × 0,6 × 3 × 0,25 = 6,4 Пбайт` на передачу в изначальном качестве с вероятностью 0,25  
`169,62 Кбайт/с × 3600 секунд × 2 411 895 × 2 × 0,6 × 3 × 0,75 = 1,8 Пбайт` на передачу в пониженом качестве с вероятностью 0,75  
Итого `8,6 Пбайт + 6,4 Пбайт + 1,8 Пбайт = 16,83 Пбайт`  

В сумме для обеих платформ `19,33 Пбайт` трафика в сутки: `9,7 Пбайт` входящего и `9,6 Пбайт` исходящего.  
`19,33 Пбайт / 24 / 3600 = 234,6 Гбайт/с` в среднем: `9,7 Пбайт / 24 / 3600 = 117,72 Гбайт/с` входящего и `9,6 Пбайт / 24 / 3600 = 116,5 Гбайт/с` исходящего.  
`19,33 Пбайт / (2 411 895 / 168 170) = 1,3 Пбайт` максимально: `9,7 Пбайт / (2 411 895 / 168 170) = 692,6 Тбайт` входящего и `9,6 Гбайт / (2 411 895 / 168 170) = 685,42 Тбайт` исходящего.
#### Аудио
...
Скорее всего, в основном будет доступно и достаточно широкополосное кодирование (16 Кбит/с). `16 Кбит/с × 3600 секунд × (2 411 895 × 4) × 0,1 × 4 = 25,88 Тбайт` трафика в сутки: `6,47 Тбайт` входящего, `19,41 Тбайт` исходящего.  
`25,88 Тбайт / 24 / 3600 = 314 Мбайт/c` в среднем: `78,5 Мбайт/с` входящего, `235,5 Мбайт/с` исходящего.  
`25,88 Тбайт / (2 411 895 / 168 170) = 1,8 Тбайт` максмально: `461,9 Гбайт` входящего, `1 385,8 Гбайт` исходящего.
#### Остальное
Если взять 512 Кбайт за средний размер HTTP запроса, получим `279 RPS × 512 Кбайт × 2 = 279 Мбайт/c` в среднем, `1 681 700 RPS × 512 Кбайт × 2 = 1 642,3 Гбайт/с` максимум.
#### Итого
...
### Оперативная память
В оперативной памяти нужно хранить `13,93 Гбайт видео + 78,5 Мбайт аудио = 14 Гбайт` по средней нагрузке и `822,93 Тбайт видео + 961,9 Гбайт аудио = 823,87 Тбайт` по максимальной нагрузке суммарно. В пересчёте на один звонок — `5 Гбайт` по максимальной нагрузке.
### Оборудование
Посчитаем средний входящий и исходящий трафик только для видео и аудио на один звонок.  
Входящий: `(9,7 Пбайт + 6,47 Тбайт) / 2 411 895 / 3600 =  1,2 Мбайт/с = 9,6 Мбит/с`.  
Исходящий: `(9,6 Пбайт + 19,41 Тбайт) / 2 411 895 / 3600 =  1,19 Мбайт/с = 9,5 Мбит/с`.

Подводя итог, на каждый звонок нужно иметь 17 потоков, 5 Гбайт оперативной памяти и 1 процессор, занятый перекодированием. По максимальному количеству звонков — 2 858 890 потоков, 840 850 Гбайт оперативной памяти и 168 170 процессоров, занятых перекодированием. На один 96-ядерный процессор помещается 11 звонков. 11 звонкам требуется 55 Гбайт оперативной памяти.  

| Роль                                     | CPU     | ОЗУ       | Жёсткий диск | Количество |
| ---------------------------------------- | ------- | --------- | ------------ | ---------- |
| Балансировщик                            | 96 (×2) | 20 Гбайт  | 480 Гбайт    | 18         |
| Сервис авторизации и управления звонками | 96 (×2) | 20 Гбайт  | 480 Гбайт    | 368        |
| СУБД                                     | 96 (×2) | 6 Гбайт   | 480 Гбайт    | 31         |
| СУБД (реплика)                           | 48 (×2) | 6 Гбайт   | 480 Гбайт    | 62         |
| Хост звонков                             | 96 (×2) | 128 Гбайт | 480 Гбайт    | 14 890     |
| Перекодировщик                           | 96 (×2) | 128 Гбайт | 480 Гбайт    | 84 085     |

## Выбор хостинга / облачного провайдера и расположения серверов
Свои сервера чтобы иметь нужную пропускную способность сети. Расположить стоит в соответствии с плотностью населения. Половину в Москве, четверть в Санкт-Петербурге, остальное равномерно распределить по Кавказу, европейской части, Уралу, Сибири, Дальнему Востоку.
## Схема балансировки нагрузки
Первоначальная балансировка с помощью geo based DNS. В каждом дата-центре запросы к сервису авторизации и управления звонками балансируются по round-robin, запросы к сервису звонков балансируются по weighted round robin, где вес зависит от количества активных звонков на этом сервере. 
